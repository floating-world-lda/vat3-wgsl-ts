...
fn computeUVs(texCoord: vec2<f32>, animData: Vat3_AnimationData, isHdr: bool, activePixelsRatio: vec2<f32>) -> Vat3_UVs {
  let scaledU = texCoord.x * activePixelsRatio.x;
  let samplingVThisFrame = (texCoord.y + animData.animProgressThisFrame) * activePixelsRatio.y;
  let samplingVNextFrame = (texCoord.y + animData.animProgressNextFrame) * activePixelsRatio.y;

  let clampedThisFrameV = fract(samplingVThisFrame);
  let clampedNextFrameV = fract(samplingVNextFrame);

  let finalThisFrameV = select(clampedThisFrameV, 1.0 - clampedThisFrameV, isHdr);
  let finalNextFrameV = select(clampedNextFrameV, 1.0 - clampedNextFrameV, isHdr);

  let thisFrameUv = vec2<f32>(scaledU, finalThisFrameV);
  let nextFrameUv = vec2<f32>(scaledU, finalNextFrameV);
...
fn isVatPaddingVertex(texCoord: vec2<f32>, frameCount: f32) -> bool {
...
fn vatLoad(tex: texture_2d<f32>, uv: vec2<f32>) -> vec4<f32> {
  let sz = textureDimensions(tex, 0);
  return textureLoad(tex, vec2<i32>(vec2<f32>(sz) * uv), 0);
}

fn sampleVat3TexturesFromParams(
  ...
  result.colorThisFrame = vatLoad(vatColTex, thisFrameUv);
  result.posThisFrame   = vatLoad(vatPosTex, thisFrameUv);
  result.rotThisFrame   = vatLoad(vatRotTex, thisFrameUv);
  ...
  if (noLerp) {
    ...
    result.colorNextFrame = vatLoad(vatColTex, nextFrameUv);
    result.posNextFrame   = vatLoad(vatPosTex, nextFrameUv);
    result.rotNextFrame   = vatLoad(vatRotTex, nextFrameUv);
  }

  if (useLookup) {
    result.lookupThisFrame = vatLoad(vatLookupTex, thisFrameUv);
    result.lookupNextFrame = select(
      vatLoad(vatLookupTex, nextFrameUv), result.lookupThisFrame, noLerp);
  }

  if (usePos2) {
    result.pos2ThisFrame = vatLoad(vatPos2Tex, thisFrameUv).xyz;
    result.pos2NextFrame = select(
      vatLoad(vatPos2Tex, nextFrameUv).xyz, result.pos2ThisFrame, noLerp);
  }

  if (useSpareColor) {
    result.spareColorThisFrame = vatLoad(vatSpareColTex, thisFrameUv);
    result.spareColorNextFrame = select(
      vatLoad(vatSpareColTex, nextFrameUv), result.spareColorThisFrame, noLerp);
  }
