...
export default abstract class VatMaterial extends MaterialPluginBase {
    ...
    if (shaderType === 'vertex') {
      return {
        CUSTOM_VERTEX_DEFINITIONS: `
            ${this.getShaderIncludeString('vat3Macros')}
            ${this.getShaderIncludeString('vat3Maths')}
            ${this.getShaderIncludeString('vat3Samplers')}
            ${this.getShaderIncludeString('vat3Structs')}
            ${this.getShaderIncludeString('vat3Vars')}
            ${this.getShaderIncludeString('vat3Utils')}
            ${this.getShaderIncludeString('vat3Wrappers')}
          `,
  ...
  public getShaderIncludeString(includeName: string): string {
    const include = ShaderStore.IncludesShadersStoreWGSL[includeName];
    if (typeof include !== 'string') {
      throw new Error(
        `VatMaterial: Shader include [${includeName}] not found in ShaderStore.IncludesShadersStoreWGSL.`
      );
    }
