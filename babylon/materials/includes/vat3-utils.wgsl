...
fn computeUVs(texCoord: vec2<f32>, animData: Vat3_AnimationData) -> Vat3_UVs {
  ...
  let clampedThisFrameV = clamp(samplingVThisFrame, 0.0, 1.0);
  let clampedNextFrameV = clamp(samplingVNextFrame, 0.0, 1.0);
...
fn sampleVat3Textures(
  ...
  if (useLookup) {
    ...
    result.lookupNextFrame = select(
      result.lookupThisFrame, textureSampleLevel(vatLookupTex, vatLookupTexSampler, nextFrameUv, 0.0), noLerp);
  }

  if (usePos2) {
    result.pos2ThisFrame = textureSampleLevel(vatPos2Tex, vatPos2TexSampler, thisFrameUv, 0.0).xyz;
    result.pos2NextFrame = select(
      result.pos2ThisFrame, textureSampleLevel(vatPos2Tex, vatPos2TexSampler, nextFrameUv, 0.0).xyz, noLerp);
  }
  ...
  if (useSpareColor) {
    ...
    result.spareColorNextFrame = select(
      result.spareColorThisFrame, textureSampleLevel(vatSpareColTex, vatSpareColTexSampler, nextFrameUv, 0.0), noLerp);
  }
