...
def data(node):
    ...
    f1 = node.evalParm("f1")
    f2 = node.evalParm("f2")
    frame_count = int(f2 - f1 + 1)

    mode = node.evalParm("mode")
    vat_type = (
        "Softbody" if mode == 0 else
        "Rigidbody" if mode == 1 else
        "DynamicMesh" if mode == 2 else
        "Particles"
    )
    ...
    if mode == 3:  # particles
        try:
            if geo.findGlobalAttrib("__max_particle_count"):
                vertex_count = int(geo.attribValue("__max_particle_count"))
            else:
                vertex_count = node.evalParm("targettexsize")
        except:
            vertex_count = 0
    else:
        try:
            input_geo = node.inputs()[0].geometry() if node.inputs() else None
            if input_geo:
                vertex_count = len(input_geo.points())
            else:
                vertex_count = len(geo.points())
        except:
            vertex_count = 0
                
    use_hdr_textures = int(node.evalParm("normalizedata") == 0)
    ...
    data.append({
        ...
        'Spare Color Texture' : node.evalParm('enable_col2'),
        'Two Position Textures' : node.evalParm('splitpos'),
        'Use HDR Textures' : use_hdr_textures,
        'VAT Type' : vat_type,
        'Vertex Count' : int(vertex_count),
    })
