...
export default class VatMetadataProcessor {
  static processMetadata(metadata: VatMetadata): {
    ...
    if (metadata['Axis System']) {
      const axisSystem = metadata['Axis System'];
      switch (axisSystem) {
        case 'Right-Handed Y-Up':
          overrides.useRightHandedCoordinates = true;
          break;
        case 'Left-Handed Y-Up':
          overrides.useRightHandedCoordinates = false;
          break;
        default:
          throw new Error(
            `VatMetadataProcessor: Unsupported coordinate system: "${axisSystem}". Expected "Right-Handed Y-Up" or "Left-Handed Y-Up"`
          );
      }
    }
