...
fn mat3Inverse(m: mat3x3<f32>) -> mat3x3<f32> {
  let a = m[0][0]; let b = m[1][0]; let c = m[2][0];
  let d = m[0][1]; let e = m[1][1]; let f = m[2][1];
  let g = m[0][2]; let h = m[1][2]; let i = m[2][2];
  let r = 1.0 / (a * (e*i - h*f) - b * (d*i - g*f) + c * (d*h - g*e));
  return mat3x3<f32>(
    vec3<f32>( (e*i - h*f) * r, (h*c - b*i) * r, (b*f - e*c) * r ),
    vec3<f32>( (g*f - d*i) * r, (a*i - g*c) * r, (d*c - a*f) * r ),
    vec3<f32>( (d*h - g*e) * r, (g*b - a*h) * r, (a*e - b*d) * r ),
  );
}
