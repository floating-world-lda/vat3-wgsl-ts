...
import { getDefaultSampler } from '../utils/texture';

export default abstract class VatMaterial extends MaterialPluginBase {
  protected _isEnabled: boolean = true;
  private readonly _textureKeys: string[];
  ...
  constructor(
    ...
    // cache texture keys
    this._textureKeys = Object.keys(this._textures);

    this.isEnabled = true;
  }

  public get emissiveTexture(): Texture {
    return this._textures.vatEmissiveTex.texture;
  }

  public set emissiveTexture(texture: Texture) {
    this._textures.vatEmissiveTex = {
      sampler: this._textures.vatEmissiveTex?.sampler ?? getDefaultSampler(),
      texture,
    };
  }
  ...
  public bindForSubMesh(
    ...
    for (const key of this._textureKeys) {
      uniformBuffer.setTexture(key, this._textures[key].texture);
    }
